...
class OcrToTableTool:
    ...
    def execute(self):
        ...
        self.store_process_image(
            '1_contours.jpg', self.image_with_contours_drawn)
        self.convert_contours_to_bounding_boxes()
        self.store_process_image(
            '2_bounding_boxes.jpg', self.image_with_all_bounding_boxes)
    ...
    def dilate_image(self):
        ...
        self.dilated_image = cv2.dilate(
            self.thresholded_image, kernel_to_remove_gaps_between_words, iterations=5)
    ...
    def convert_contours_to_bounding_boxes(self):
        ...
        for contour in self.contours:
            ...
            self.image_with_all_bounding_boxes = cv2.rectangle(
                self.image_with_all_bounding_boxes, (x, y), (x + w, y + h), (0, 255, 0), 5)
    ...
    def crop_each_bounding_box_and_ocr(self):
        ...
        for row in self.rows:
            for bounding_box in row:
                ...
                image_slice_path = "./ocr_slices/img_" + \
                    str(image_number) + ".jpg"
                cv2.imwrite(image_slice_path, cropped_image)
                results_from_ocr = self.get_result_from_tersseract(
                    image_slice_path)
    ...
    def get_result_from_tersseract(self, image_path):
        output = subprocess.getoutput(
            'tesseract ' + image_path + ' - -l eng --oem 3 --psm 7 --dpi 72 -c tessedit_char_whitelist="ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789().calmg* "')
        output = output.strip()
        return output
